...
async function main() {
  try {
    ...
    if (!session) {
      ...
      if (!draft) {
        console.log('No DRAFT session found either!');
        return;
      }

      console.log(`Marking DRAFT session ${draft.id} as SUBMITTED...`);
      ...
      draft.additionalInfoPending = JSON.stringify({
        ...
        frequency: '50 Hz',
        hvSide: '150 kV',
        hvRatedCurrent: '115.4 A',
        lvSide: '20 kV',
        lvRatedCurrent: '866 A'
      });
      session = await sessionRepo.save(draft);
      console.log('Session marked as SUBMITTED.');
    }

    console.log(`Approving session ID: ${session.id}...`);
    ...
    session.validatedById = 'c90c7605-bf78-4d07-bf78-20c760560a53'; // Mock user ID
    ...
    const auditLog = auditRepo.create({
      userId: 'c90c7605-bf78-4d07-bf78-20c760560a53',
    ...
    console.log('--- APPROVAL SIMULATION SUCCEEDED ---');
  } catch (err) {
